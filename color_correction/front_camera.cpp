...
Mat cartesianRotate(Mat image, int method) {
  ...
  Mat rotated =
      Mat(image.rows, image.cols, CV_MAKETYPE(CV_8U, image.channels()));
...
int main() {
  namedWindow("F.C.Image", WINDOW_NORMAL);
  namedWindow("GammaCorr", WINDOW_NORMAL);
  namedWindow("C.C.", WINDOW_NORMAL);
  namedWindow("BuoyContours", WINDOW_AUTOSIZE);
  // namedWindow();
      ...
      for (int p = 0; p < img.rows - img.rows / (64 * mul);
        ...
        for (int q = 0; q < img.cols - img.cols / (48 * mul);
          ...
          for (int i = p; i < p + img.rows / (64 * mul); i++) {
            for (int j = q; j < q + img.cols / (48 * mul); j++) {
              Vec3b color = g_corr.at<Vec3b>(i, j);
              if (2 * color[1] >= color[2] + color[0] + iter &&
                  gavg >= 45) //&&2*color[1]>color[2]+color[0])
              {
                color[1] = 255;
                color[0] = 0;
                color[2] = 255;
              } else if (2 * color[2] >= color[1] + color[0] + iter + 15 &&
                         ravg >= 65) //&&2*color[2]>color[1]+color[0])
              {
                color[1] = 0;
                color[0] = 0;
                color[2] = 255;
...
int ballDetect(Mat img2) {
  ...
  for (int i = 0; i < no_of_bases; i++) {
    ...
    calcHist(&imgf_base[i], 1, channels, Mat(), histograms_f[i], 2, histSize,
            ranges, true, false);
    normalize(histograms_f[i], histograms_f[i], 0, 1, NORM_MINMAX, -1, Mat());
  }
  calcHist(&imgf_test, 1, channels, Mat(), histograms_bot, 2, histSize, ranges,
          true, Mfalse);
  ...
  for (int i = 0; i < no_of_bases; i++) {
    ...
    compare_values[i] =
        compareHist(histograms_f[i], histograms_bot, compare_method);
