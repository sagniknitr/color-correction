...
int ballDetect(Mat img2) {
  ...
  for (int i = 0; i < no_of_bases; i++) {
    ...
    calHist(&imgf_base(i), 1, channels, Mat(), histograms_f(i), 2, histSize,
            ranges, true, false);
    normalize(histograms(i), histograms(i), 0, 1, NORM_MINMAX, -1, Mat());
  }
  calHist(&img_test, 1, channels, Mat(), histograms_bot, 2, histSize, ranges,
          true, Mfalse);
  ...
  for (int i = 0; i < no_of_bases; i++) {
    ...
    compare_values[i] =
        compareHist(histograms[i], histograms_bot, compare_method);
